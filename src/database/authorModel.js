...
const addPost = async (title, content, isPremium = false) => {
  try {
    const query = `INSERT INTO posts (title, content, is_premium, created_at) VALUES ($1, $2, $3, NOW()) RETURNING *;`;
...
const editPost = async (postId, title, content, isPremium) => {
  try {
    const query = `UPDATE posts SET title = $1, content = $2, is_premium = $3, created_at = NOW() WHERE id = $4 RETURNING *;`;
...
const updateUserSubscription = async (userId, isPremium) => {
  try {
    const query = `UPDATE users SET is_premium = $1 WHERE id = $2 RETURNING *;`;
